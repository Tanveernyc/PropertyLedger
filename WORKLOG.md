...
## Phase 17 — Social sign-in (in progress)
- START: 2026-09-22T00:00:00Z — agent: claude — Sign in with Apple and Google (plan docs/superpowers/plans/2026-09-22-social-sign-in.md).
- BLOCKER: `main` must NOT be built for distribution until docs/setup-social-sign-in.md §1–§3 are complete (Apple App ID capability, Google OAuth clients, both Supabase providers enabled) and the two env flags are set — `EXPO_PUBLIC_APPLE_SIGN_IN_ENABLED=true` plus the two `EXPO_PUBLIC_GOOGLE_*_CLIENT_ID` values. Both providers fail closed: with the flags unset neither button renders, which is the intended state until that console work lands.
